...
2. Go to Router01, on the top button bar is a "Connect" drop-down button, click it and select the Bastion option.
3. 10.10.4.0/26 should be the next select address range for the Bastion prefix, accept whatever is there. Leave the NSG option as "none", click "Create Subnet" button.
4. In Step 3, keep all the default options and click the "Create Azure Bastion using defaults" button to start the creation process.
5. Creating the bastion host will take 10 - 15 minutes.
6. When complete, return to the connect button on the Router and select Bastion again. You should now be able to SSH (in your browser) to the router.
7. You can do the same to Router02 (once the Bastion is created it will work for all VMs and NVAs in your VNet) which will open in a new tab.
